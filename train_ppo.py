...
# Path to precomputed Word2Vec table (.npz). Set via --w2v-table.
W2V_TABLE: str | None = None

def make_env():
    device = "cuda" if torch.cuda.is_available() else "cpu"
    resnet = ResNet50Encoder(device=device)
    print("resnet encoder device attr:", getattr(resnet, "device", None), flush=True)
    # Use precomputed W2V table to avoid loading 3.4GB GoogleNews vectors.
    if W2V_TABLE is not None:
        print(f"[W2V] Using table: {W2V_TABLE}", flush=True)
        embed = WordEmbed(table_path=W2V_TABLE)
    else:
        print("[W2V] WARNING: --w2v-table not set; this may load the 3.4GB model (slow / OOM risk).", flush=True)
        embed = WordEmbed()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to precomputed w2v table (.npz) generated by get_all_w2v.py. "
             "If provided, the env will NOT load the 3.4GB GoogleNews vectors.",
    )
    args = parser.parse_args()

    global W2V_TABLE
    W2V_TABLE = args.w2v_table
